-- Follow-up to 010: restore migration 005's hardened search_path
-- (public, extensions) on create_puja_booking and drop the pgcrypto
-- dependency for the confirmation ref — gen_random_uuid() is a pg_catalog
-- builtin in Postgres 13+ and needs no schema extension.
...
begin
  ...
  values (auth.uid(), o.id, o.temple_id, p_booking_date, p_quantity, o.price * p_quantity, o.currency, coalesce(p_sankalpa, '{}'::jsonb), coalesce(p_family_member_ids, '{}'), p_notes, coalesce(p_prasadam, 'none'),
          ...
          'PRT-' || upper(substr(replace(gen_random_uuid()::text, '-', ''), 1, 8)))
  returning * into b;
